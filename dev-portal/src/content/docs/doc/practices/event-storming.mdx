...
the ones registered in the [event catalogue](/go/events). What the workshop does
not produce is an order of work: for that, the same room runs
[story mapping](/doc/practices/story-mapping/), which lays the user's journey out
and cuts it into releases. The individual stories then go to
