---
title: Story mapping
description: Laying the user's journey out along a backbone and slicing it into releases, so a backlog keeps the narrative a flat list destroys — and the move from narrative speech into the story grammar.
---
...
<figure>
  <img
    src="/img/practices/story-mapping.svg"
    alt="A story map: a backbone of five user activities across the top, tasks stacked underneath in order of necessity, and dashed horizontal cuts marking releases — the first crossing every activity — with a red line above the items nobody has committed to."
    width="820"
    height="392"
  />
  <figcaption>
    User story mapping was created by <strong>Jeff Patton</strong>, first in his
    2005 article <em>It's All in How You Slice It</em> and then in the book{' '}
    <em>User Story Mapping: Discover the Whole Story, Build the Right Product</em>.
    A full guide is at{' '}
    <a href="https://www.avion.io/what-is-user-story-mapping/">avion.io/what-is-user-story-mapping</a>.
  </figcaption>
</figure>
...
The file is what the tracker, and eventually the pipeline, reads. See
[digital artefacts](/doc/practices/digital-artefacts/) for the whole chain and
for the direction rule that keeps it trustworthy.
...
supports [ticket emission](/doc/practices/story-tickets/) — a ticket is a
scheduling record of *one thing someone wants*, and the story grammar is that
sentence already in the right shape.
...
picked up one session later, in
